...
def get_crypto_map():
    if os.path.exists('top200_yahoo.csv'):
        df = pd.read_csv('top200_yahoo.csv')
...
def fetch_ticker(args):
    ...
    try:
        ...
        df = df.iloc[:-1] # Jan 26 Confirmed Close
...
with t_stocks:
    ...
    st.caption("📅 Daily Close: Jan 26, 2025")
...
with t_coins:
    ...
    st.caption(f"📅 Daily Close: Jan 26, 2025 | Coins Loaded: {len(df_c)}")
